...
#         self.login_user.remove(user)
#
#     def show_users_login(self):
#         for user in self.login_user:
#             print user
# login_engine = login_useres()


# def user_validation(name):
#     result = s.query(User.username).filter_by(username = name).first()
#     return result == None

# def pwd_checking(pwd):
    ...
    if request.method == 'POST':
        ...
        if user:
            if check_password_hash(user.password, request.form['pwd']):
                ...
                s.add(user_login)
                s.commit()
                # login_engine.login(user.username)

                return redirect(url_for('home', user_login=user.username))
...
def home():
    ...
    if fullname[0] == 'Unset':
        return redirect(url_for('basic_profile', user_login=username))
    ...
    return render_template('home.html', username=username, x=x, chart=chart)
